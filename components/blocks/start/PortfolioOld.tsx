...
export default function Portfolio({ portfolio }: HeroProps) {
	...
	const left = useTransform(scrollYProgress, [-1000, 1000], [0, 0]);
	...
	return (
		<section className={cn(s.portfolio)} ref={ref} data-lenis-snap={true}>
			...
			>
				{portfolio.concat(portfolio).map(({ id, image, name, text }, idx) => (
					<SwiperSlide key={idx} onClick={() => swiperRef.current?.slideNext()} className={s.slide}>
						{image && (
							<figure>
